...
class MrpCostStructure(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        if data.get('context', {}).get('mo_wise_cost_analysis', ''):
            productions = self.env['mrp.production'] \
                .browse(docids) \
                .filtered(lambda p: p.state != 'cancel')
            res = None
            if all(production.state == 'done' for production in productions):
                res = self.with_context(mo_wise_cost_analysis=True).get_lines(productions)
            return {'lines': res}
        return super(MrpCostStructure, self)._get_report_values(docids)

    def get_lines(self, productions):
        if self.env.context.get('mo_wise_cost_analysis', False):
            ProductProduct = self.env['product.product']
            StockMove = self.env['stock.move']
            res = []
            for product in productions.mapped('product_id'):
                product_orders = productions.filtered(lambda m: m.product_id == product)
                for mos in product_orders:
                    total_cost = 0.0
                    ...
                    if any(m.product_uom_id.id != uom.id for m in mos):
                        uom = product.uom_id
                        for m in mos:
                            qty = sum(m.move_finished_ids.filtered(
                                lambda mo: mo.state == 'done' and mo.product_id == product).mapped('product_uom_qty'))
                        ...
                        for m in mos:
                            mo_qty += sum(m.move_finished_ids.filtered(
                                lambda mo: mo.state == 'done' and mo.product_id == product).mapped('product_uom_qty'))
                    for m in mos:
            ...
            return res
        return super(MrpCostStructure, self).get_lines(productions)
